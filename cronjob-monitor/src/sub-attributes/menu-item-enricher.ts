...
import { SubAttributesChange, MenuItemInfo } from '../types';

/**
 * Enrich added and modified sub-attributes with menu item information from BJs API
 * @param changes Array of sub-attributes changes (added and/or modified)
 * @returns Promise with enriched sub-attributes changes
 */
export async function enrichAddedSubAttributesWithMenuItems(
  changes: SubAttributesChange[]
): Promise<SubAttributesChange[]> {
  const enriched: SubAttributesChange[] = [];
  ...
  for (const change of changes) {
    ...
    // Enrich if it's an added or modified sub-attribute with valid parsed IDs
    if (
