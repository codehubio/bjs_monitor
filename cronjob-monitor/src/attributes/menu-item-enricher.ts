...
import { AttributesChange, MenuItemInfo } from '../types';

/**
 * Enrich added and modified attributes with menu item information from BJs API
 * @param changes Array of attributes changes (added and/or modified)
 * @returns Promise with enriched attributes changes
 */
export async function enrichAddedAttributesWithMenuItems(
  changes: AttributesChange[]
): Promise<AttributesChange[]> {
  const enriched: AttributesChange[] = [];
  ...
  for (const change of changes) {
    ...
    // Enrich if it's an added or modified attribute with valid parsed IDs
    if (
