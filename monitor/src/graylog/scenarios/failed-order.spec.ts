...
import queries, { GROUP_BY_COLUMN_1, GROUP_BY_COLUMN_2, GROUP_BY_COLUMN_3, GROUP_BY_COLUMN_4 } from '../searchText/failed-order';
...
test.describe('Failed Order Search', () => {
  test('should login, report failed orders, wait for results', async ({ page }) => {
    ...
    const searchViewId = (config as any).graylogFailedOrderSearchView || config.graylogSubmitOrderSearchView;
    if (!searchViewId) {
      throw new Error('GRAYLOG_FAILED_ORDER_SEARCH_VIEW or GRAYLOG_SUBMIT_ORDER_SEARCH_VIEW environment variable is not set');
    }
    ...
    const resultsDir = path.resolve(process.cwd(), 'src','graylog','result', 'failed-order', datetimeFolder);
    ...
    for (let i = 0; i < queries.length; i++) {
      ...
      // Execute query using Graylog API client and wait for results (grouped by 4 columns)
      let groupedData: any[] = [];
      ...
      try {
        ...
        console.log(`API Query Grouped Results (4 columns):`, groupedData);
    ...
    try {
      ...
      await uploadFolderToS3(resultsDir, `${s3Prefix}/failed-order`);
      const fullPrefix = s3Prefix ? `${s3Prefix}/failed-order` : 'failed-order';
    ...
    // Note: Using buildAndSendGroup1ColumnAdaptiveCard as a fallback since 2-column support doesn't exist yet
    // The grouped data will still be in the results, but the card will only show the first column
    if (config.msTeamWebhookUrl) {
      try {
        ...
        const title = `Failed Order Report - ${fromTime} to ${toTime}`;
        
        // Pass additional columns to display in sub-table (columns 3 and 4 have 1-1-1 mapping with column 2)
        await buildAndSendGroup2ColumnAdaptiveCard(
          ...
          GROUP_BY_COLUMN_2, 
          urls,
          [GROUP_BY_COLUMN_3, GROUP_BY_COLUMN_4]
        );
